...
The final versions of the files created in this section can be found in [this repo](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton)
...
<AnchorLinks small>
  ...
  <AnchorLink>Code changes</AnchorLink>
  <AnchorLink>Create the WebSphere Liberty server.xml file</AnchorLink>
  <AnchorLink>Handling environment specific configuration</AnchorLink>
  <AnchorLink>Monitoring</AnchorLink>
  <AnchorLink>Project structure</AnchorLink>
  <AnchorLink>Create the Docker Dockerfile</AnchorLink>
  <AnchorLink>Run the application locally</AnchorLink>
</AnchorLinks>
...
In this section, the `server.xml` file to prepare the Liberty server to run the Customer Order Services application will be reviewed. The file can be found [here](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/liberty/server.xml). and is shown below:
...
In a native development environment (non-Docker) a `server.env` file can be used to inject the environment variables directly in to WebSphere Liberty when it starts. This avoids having to set environment variables on the development machine. This file is placed in the same folder as the `server.xml` file and is read by Liberty on startup. The file can be found [here](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/liberty/server.env). and is shown below:
```
...
### Monitoring
[Prometheus](https://prometheus.io/) and [Grafana](https://grafana.com) are widely used in Kubernetes to monitor applications. Prometheus is a systems and service monitoring system. It collects metrics from configured targets at given intervals, evaluates rule expressions, displays the results, and can trigger alerts if some condition is observed to be true.
...
The JMX exporter is made up of a [JAR file](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/resources/jmx_exporter) and a [configuration file](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/resources/jmx_exporter/jmx-config.yml). The configuration file used for the Customer Order Services application is shown below.
...
In order to connect the JMX exporter and configuration file to Liberty on startup, a `jvm.options` file is required. The [jvm.options](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/liberty/jvm.options) file is shown below.
...
The `Dockerfile` for the Customer Order Services application is shown below. The file can be found [here](https://github.com/ibm-cloud-architecture/appmod-liberty-tekton/blob/master/Dockerfile)
```
...
IBM provides a set of WebSphere Liberty [Docker images](https://hub.docker.com/_/websphere-liberty). This scenario uses the Docker images that are based on the [RedHat Universal Base Image](https://www.redhat.com/en/blog/introducing-red-hat-universal-base-image). The `ibmcom/websphere-liberty:kernel-java8-ibmjava-ubi` image contains only the smallest components of the Liberty server. The features used by the application are loaded in to the Docker image later in the `RUN configure.sh` step.
...
```
git clone https://github.com/ibm-cloud-architecture/appmod-liberty-tekton
...
- Modify the `liberty/server.env` file for your environment as shown below:
``` bash
DB2_HOST=172.16.52.252
DB2_PORT=50000
DB2_DBNAME=ORDERDB
...
- Access the application in a browser using `https://127.0.0.1:9443/CustomerOrderServicesWeb`. Login using `rbarcia` and `bl0wfish` and then add an item to the shopping cart
  ![App](images/liberty-build/app.jpg)


- Validate that the `/metrics` endpoint is available at `http://127.0.0.1:9081/metrics`
...
Now proceed to the either the [Liberty - Deploy using Cloud Native Toolkit](liberty-deploy-cloud-native-toolkit) section or the [Liberty - Deploy using OpenShift Pipelines](liberty-deploy-openshift-pipelines) section where the process of automating the deployment of the application to a Kubernetes runtime will be covered step-by-step
